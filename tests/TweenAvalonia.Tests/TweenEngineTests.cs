...
public class TweenEngineTests
{
    ...
    {
        double value = 0;
        Tween tween = Tween.Custom(0d, 1d, v => value = v, Duration, Linear);
    ...
    [Test]
    public void UnhandledException_OnCompleteThrow_RaisedOnEvent()
    {
        var target = new TestTarget();
        Exception? raised = null;
        Tween.UnhandledException += ex => raised = ex;
        ...
        try
        {
            ...
            Assert.That(raised!.Message, Is.EqualTo("boom"));
        }
        finally
        {
            Tween.UnhandledException -= ex => raised = ex;
